...
if [ ! -f "$RESULTS_FILE" ]; then
    echo "depth,vocab_size,model_dim,num_params,num_scaling_params,num_iterations,tokens_trained,param_data_ratio,val_bpb,core_score,train_time_sec" > "$RESULTS_FILE"
fi
...
for vocab_size in "${VOCAB_SIZES[@]}"; do
    python -m scripts.tok_train --max-chars=2000000000 --vocab-size=$vocab_size
    for d in "${DEPTHS[@]}"; do
        log "Training d=$d, vocab_size=$vocab_size..."

        TAG="${SERIES_NAME}_miniseries_d${d}vocab${vocab_size}"
        START_TIME=$(date +%s)
        ...
        torchrun --standalone --nproc_per_node=$NPROC_PER_NODE -m scripts.base_train -- \
            ...
            --run="${WANDB_RUN}_d${d}vocab${vocab_size}" \
            --model-tag="${TAG}" \
        ...
        log "  d=$d, vocab_size=$vocab_size:  params=$NUM_PARAMS, scaling=$NUM_SCALING_PARAMS, ratio=$PARAM_DATA_RATIO, bpb=$VAL_BPB, CORE=$CORE_SCORE, time=${TRAIN_TIME}s"

        # Append to CSV
        echo "$d,$vocab_size,$MODEL_DIM,$NUM_PARAMS,$NUM_SCALING_PARAMS,$NUM_ITERS,$TOKENS_TRAINED,$PARAM_DATA_RATIO,$VAL_BPB,$CORE_SCORE,$TRAIN_TIME" >> "$RESULTS_FILE"
    done
done
