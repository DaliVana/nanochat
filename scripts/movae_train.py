"""
Train Mixture of Value and Experts (MoVaE) model. From root directory of the project, run as:

python -m scripts.movae_train

or distributed as:

torchrun --nproc_per_node=8 -m scripts.movae_train

If you are only on CPU/Macbook, you'll want to train a much much smaller LLM. Example:
python -m scripts.movae_train --depth=4 --max-seq-len=512 --device-batch-size=1 --eval-tokens=512 --core-metric-every=-1 --total-batch-size=512 --num-iterations=20
python -m scripts.movae_train --depth=8 --max-seq-len=512 --device-batch-size=8 --eval-tokens=2048 --core-metric-every=-1 --total-batch-size=2048 --num-iterations=100
"""
...
from nanochat.gpt_movae import GPTMoVaE, GPTConfigMoVaE
...
# -----------------------------------------------------------------------------
# CLI arguments
parser = argparse.ArgumentParser(description="Pretrain Mixture of Value and Experts (MoVaE) model")
# Logging
...
# Mixture of Experts (MoE) - MLP expert routing
...
output_dirname = args.model_tag if args.model_tag else f"movae_d{args.depth}" # e.g. movae_d12
checkpoint_dir = os.path.join(base_dir, "movae_checkpoints", output_dirname)
