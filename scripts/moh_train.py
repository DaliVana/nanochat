"""
Train Mixture of Heads (MoH) model. From root directory of the project, run as:

python -m scripts.moh_train

or distributed as:

torchrun --nproc_per_node=8 -m scripts.moh_train

If you are only on CPU/Macbook, you'll want to train a much much smaller LLM. Example:
python -m scripts.moh_train --depth=4 --max-seq-len=512 --device-batch-size=1 --eval-tokens=512 --core-metric-every=-1 --total-batch-size=512 --num-iterations=20
"""
...
from nanochat.gpt_moh import GPTMoH, GPTConfigMoH
...
# -----------------------------------------------------------------------------
# CLI arguments
parser = argparse.ArgumentParser(description="Pretrain Mixture of Heads (MoH) model")
# Logging
...
output_dirname = args.model_tag if args.model_tag else f"moh_d{args.depth}" # e.g. moh_d12
checkpoint_dir = os.path.join(base_dir, "moh_checkpoints", output_dirname)
