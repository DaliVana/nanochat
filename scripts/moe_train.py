"""
Train Mixture of Experts (MoE) model. From root directory of the project, run as:
...
python -m scripts.moe_train --depth=4 --max-seq-len=512 --device-batch-size=1 --eval-tokens=512 --core-metric-every=-1 --total-batch-size=512 --num-iterations=20
"""
...
# -----------------------------------------------------------------------------
# CLI arguments
parser = argparse.ArgumentParser(description="Pretrain Mixture of Experts (MoE) model")
# Logging
...
output_dirname = args.model_tag if args.model_tag else f"moe_d{args.depth}" # e.g. moe_d12
checkpoint_dir = os.path.join(base_dir, "moe_checkpoints", output_dirname)
