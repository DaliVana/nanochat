...
class TopKRouter(nn.Module):
    ...
    def forward(self, x):
        ...
        num_tokens_per_expert = torch.zeros(self.num_experts, device=flat.device, dtype=torch.float32)
        num_tokens_per_expert.scatter_add_(0, flat.long(), torch.ones(flat.shape[0], device=flat.device, dtype=torch.float32))
...
def _run_experts_for_loop(w_up, w_down, x, num_tokens_per_expert):
    ...
    for i, chunk in enumerate(chunks):
        if chunk.shape[0] == 0:
            # Skip empty chunks: MPS crashes on backward through empty-tensor matmul.
            # Safe because w_up/w_down are a single 3D Parameter — unused expert slices
            # get zero gradient automatically (not None), so stacking is unaffected.
