...
class MLACausalSelfAttention(nn.Module):
    ...
    def forward(self, x, ve, cos_sin, window_size, kv_cache):
        ...
        # cos_sin should be precomputed for d_rope dimensions
        cos, sin = cos_sin
...
class GPT(nn.Module):
    def __init__(self, config, pad_vocab_size_to=64):
        ...
        self.value_embeds = nn.ModuleDict({str(i): nn.Embedding(padded_vocab_size, ve_dim) for i in range(config.n_layer) if has_ve(i, config.n_layer)})
